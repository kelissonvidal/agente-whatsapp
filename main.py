...
def webhook():
    ...
    if etapa == 2:
        ...
        if sessao["permite_audio"]:
            ...
            enviar_mensagem(telefone, "(Texto alternativo ao áudio 1)")
        sessao["aguardando_resposta"] = True
        return jsonify({"status": "inicio_funil"})
    ...
    if etapa in [8, 9, 10]:
        ...
        time.sleep(8)
        avancar()
        return jsonify({"status": f"etapa_{etapa}_automatica"})

    if etapa == 11:
        enviar_audio(telefone, "audio_9.ogg") if sessao["permite_audio"] else enviar_mensagem(telefone, "(Texto alternativo ao áudio 9)")
        time.sleep(8)
        asyncio.run(enviar_blocos_finais(telefone))
        avancar()
        return jsonify({"status": "fechamento_final"})
